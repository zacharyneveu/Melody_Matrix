...
function setup() {
    forRange(x => forRange(y => forRange(z => {
        let indexArr = []
        indexArr.push(coordToIndex(x))
        indexArr.push(coordToIndex(y))
        indexArr.push(coordToIndex(z))
        spheres.push({ x, y, z, indexArr })
    })))
...
function getColor() {
    ...
    colorContainer = isCustom ? currentCustom : colors[modelData.genre];
}
...
function getFrame() {
    ...
    modelData.notes.forEach(note => {
        ...
        let color = colorContainer[node].map(x => x * note.energy)
...
function draw() {
    ...
    spheres.forEach(obj => {
        ...
        if (datas) {
            let tem_arr = datas[obj.indexArr[0]][obj.indexArr[1]][obj.indexArr[2]];
